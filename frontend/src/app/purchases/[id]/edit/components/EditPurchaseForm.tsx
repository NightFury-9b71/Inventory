...
import { Purchase, PurchaseFormData, PurchaseItem } from "@/types/purchase";
...
export default function EditPurchaseForm({
  ...
  const addNewItem = () => {
    ...
    const updatedItems = [...(purchase as any).items || [], newItem];
    onInputChange("items", updatedItems);
  };

  const removeItem = (index: number) => {
    const updatedItems = (purchase as any).items?.filter((_: PurchaseItem, i: number) => i !== index) || [];
    onInputChange("items", updatedItems);
  };

  const updateItem = (index: number, field: keyof PurchaseItem, value: any) => {
    const updatedItems = [...((purchase as any).items || [])];
  ...
  const calculateGrandTotal = () => {
    ...
    return items.reduce((sum: number, item: PurchaseItem) => sum + (item.totalPrice || 0), 0).toFixed(2);
  };
  ...
  const isFormValid = () => {
    ...
    const allItemsValid = items.every((item: PurchaseItem) => 
      item.itemId > 0 && 
      item.quantity > 0 && 
      item.unitPrice > 0
    );
      ...
      <CardContent>
        ...
        <form onSubmit={onSubmit} className="space-y-6">
          <div className="space-y-4">
            ...
            {((purchase as any).items || []).length === 0 && (
              <div className="text-center py-8 text-gray-500">
                No items added yet. Click "Add Item" to start.
              </div>
            )}

            {((purchase as any).items || []).map((item: PurchaseItem, index: number) => (
